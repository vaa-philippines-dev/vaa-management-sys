...
export function VACsvImportFloatingWidget() {
  const { state, restore, cancelImport, reset } = useVACsvImport()
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 w-72 rounded-xl border bg-card shadow-2xl p-3 space-y-2">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-1.5 min-w-0">
          ...
          <p className="text-xs font-medium truncate">
            {isRunning ? `Importing VAs ${pct}%` : state.status === 'cancelled' ? 'Import cancelled' : 'Import finished'}
          </p>
        </div>
      ...
      <p className="text-[11px] text-muted-foreground truncate">
        {state.fileName} · {state.processedRows}/{state.totalRows} rows

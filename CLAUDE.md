...
**VA Preparation and Performance Monitoring need a real `Assignment` to attach to.** When the database has none for a given VA-client pair, the importer creates one (`AssignmentSource.DMF_SYNC`) from the sheet's own data — PRIMARY ACCOUNT (client), ACTUAL/TARGET START DATE, NO OF HOURS, VA STATUS WITH CLIENT (→ status), EFFECTIVITY DATE (→ endDate) — and seeds its `AssignmentPreparation` row and 7 `AssignmentKpiCheck` rows the same way `createAssignment()` does, so it behaves identically to one made through the UI. `type` always defaults to `REGULAR`: no column in either tab distinguishes ongoing work from a one-off project. A row is skipped (not created) when NO OF HOURS or a start date is missing, rather than fabricating either. This was an explicit decision, not a default — the first version of this importer deliberately did *not* create Assignments, and only did after being asked to; before doing this for a new department, confirm creating ~hundreds of real staffing records from sheet text is actually wanted there too. Verified against the live Amazon sheet: 395 Assignments created, dates/hours/status spot-checked against the raw sheet, zero duplicate `ExternalSyncMapping` rows, and the two real Amazon VAs missing an EOC/pause date now correctly flagged on the dashboard (not the artificial "2,000+" false-positive count this would've been without the per-VA staleness guards already in place elsewhere).
